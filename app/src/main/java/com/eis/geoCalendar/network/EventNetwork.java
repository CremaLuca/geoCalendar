...
public interface EventNetwork<E extends NetworkEvent> {
     ...
     * @param storeListener {@link StoreEventListener#onEventStored(NetworkEvent)} is be called if the event is correctly stored,
     *                      {@link StoreEventListener#onEventStoreFail(NetworkEvent, com.eis.communication.network.FailReason)} otherwise
     */
    void storeEvent(final @NonNull E event, final @Nullable StoreEventListener<E> storeListener);
     ...
     */
    void getEvents(final @NonNull GPSPosition requestedPosition, final double radius, final @NonNull GetEventListener<E> getListener);
